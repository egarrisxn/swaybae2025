...
const LargeDock = ({ items, className }) => {
  ...
  return (
    <motion.div
      ...
      className={cn(
        "mx-auto hidden h-16 items-end gap-4 rounded-full bg-background px-6 pb-3 [box-shadow:0_0_0_1px_rgba(0,0,0,.03),0_2px_4px_rgba(0,0,0,.05),0_12px_24px_rgba(0,0,0,.05)] md:flex dark:[border:1px_solid_rgba(255,255,255,.1)] dark:[box-shadow:0_-20px_80px_-20px_#ffffff1f_inset]", // Large dock styles
        className,
...
function DockIcon({ mouseX, title, icon, href }) {
  const ref = useRef(null); // Ref for measuring distance from mouse
...
function AnimatedDock({ items, largeClassName, smallClassName }) {
  return (
    <>
...
export default function Dock() {
  return (
    <div className="fixed bottom-0 right-20 z-40 mx-auto mb-4 flex h-full max-h-24 md:inset-x-0 md:right-0">
      <div className="relative z-50 mx-auto flex h-20 flex-row items-center justify-center">
        <AnimatedDock
          items={[
            { href: "/", icon: <HomeIcon />, title: "Home" },
            { href: "/about", icon: <UserIcon />, title: "About" },
            { href: "/blog", icon: <PenLineIcon />, title: "Blog" },
            { href: "/calendar", icon: <CalendarIcon />, title: "Calendar" },
            { href: "/community", icon: <HandshakeIcon />, title: "Community" },
            { href: "/contact", icon: <MailIcon />, title: "Contact" },
            { href: "/media", icon: <ImagesIcon />, title: "Media Kit" },
            {
